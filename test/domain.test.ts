...
test('staff fraud confirmation excludes automatic duplicates and reward delivery incidents', () => {
  ...
  assert.equal(canConfirmReceiptFraud('AUTO_REJECTED', false, ['USER_DAILY_POINTS_LIMIT']), false);
  ...
  assert.equal(isValidWithoutReward('AUTO_REJECTED', ['USER_DAILY_POINTS_LIMIT']), true);
...
test('application settings validate keys, normalize line endings and enforce limits', () => {
  ...
  assert.equal(normalizeAppSettingValue('limits.dailyPointsPerUser', '100'), '100');
